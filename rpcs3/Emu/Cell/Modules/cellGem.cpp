...
namespace tracker
{
	static void init(gsl::not_null<gem_t*> gem)
	{
		...
		std::tie(settings.camera_frame_width, settings.camera_frame_height) =
			get_format_resolution(gem->vc_attribute.output_format);

		// settings.camera_frame_rate = gem->vc_attribute
